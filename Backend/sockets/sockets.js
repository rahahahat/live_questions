...
module.exports = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("questionTest", ({ questionID }) => {
      Question.findById(questionID, (err, result) => {
        console.log(result);
        io.emit("questionTest", result);
      });
    });
    // TESTING SOCKETS ----------------------------------------------------------------------------------------------- //
    ...
    socket.on("toggle-questions", (onOrOff) => {
      let switchText = onOrOff
        ? "turning questions on"
        : "turning questions off";
      ...
      Room.findOne({
        ...
        .exec((err, result) => {
          if (err || !result) {
            ...
            socket
              .to(roomUrl)
              .emit("update-user-list", CLIENTS[socket.questionRoom]);
            //----------------------------------
    ...
    socket.on("disconnect", () => {
      ...
      if (socket.questionRoom in CLIENTS) {
        CLIENTS[socket.questionRoom] = CLIENTS[socket.questionRoom].filter(
          (client) => client.id != socket.id
        );
        //console.log(CLIENTS);
        socket
          .to(socket.questionRoom)
          .emit("update-user-list", CLIENTS[socket.questionRoom]);
      }
